...
mod prng {
    ...
    pub fn rand() -> f64 {
        loop {
            ...
            if SEED.compare_exchange_weak(old, new, Ordering::Relaxed, Ordering::Relaxed).is_ok() {
...
impl Context {
    pub fn new() -> Self { Context { vars: BTreeMap::new() } }
    pub fn set(&mut self, name: &str, value: f64) { self.vars.insert(name.to_string(), value); }
    pub fn get(&self, name: &str) -> Option<f64> { self.vars.get(name).copied() }
}

impl Default for Context { fn default() -> Self { Context::new() } }
...
pub fn eval_env(expr: &Expr, env: &Env) -> Result<f64, CalcError> {
    match expr {
        ...
        Expr::Const(c) => Ok(match c {
            ...
            Constant::I     => return Err(CalcError::InvalidArgument("complex 'i' cannot be evaluated to f64".into())),
        }),
        Expr::Var(name) => {
        ...
        Expr::Add(terms) => {
            let mut acc = 0.0f64;
            for t in terms { acc += eval_env(t, env)?; }
            Ok(acc)
        }
        Expr::Mul(factors) => {
            let mut acc = 1.0f64;
            for f in factors { acc *= eval_env(f, env)?; }
            Ok(acc)
        }
        Expr::Pow(base, exp) => {
            Ok(libm::pow(eval_env(base, env)?, eval_env(exp, env)?))
        }
        Expr::Call(f, args) => eval_call(f, args, env),
        Expr::FnCall(name, args) => {
            let (params, body) = {
                let uf = env.get_fn(name)
                    .ok_or_else(|| CalcError::UndefinedVariable(format!("function '{}' not defined", name)))?;
        ...
        Expr::List(items) => {
            ...
            if items.len() == 1 { return eval_env(&items[0], env); }
            Err(CalcError::InvalidArgument("list cannot be evaluated as a scalar".into()))
        }
        Expr::Matrix(_) => Err(CalcError::InvalidArgument("matrix cannot be evaluated as a scalar".into())),
        Expr::Equation(l, r) => {
...
fn eval_call(f: &BuiltinFn, args: &[Expr], env: &Env) -> Result<f64, CalcError> {
    match f {
        ...
        BuiltinFn::Ln       => {
            ...
            if x <= 0.0 { return Err(CalcError::InvalidArgument("ln requires x > 0".into())); }
            Ok(libm::log(x))
        }
        BuiltinFn::Log      => {
            if args.len() == 2 {
        ...
        BuiltinFn::Sqrt     => {
            let x = arg(args, 0, env)?;
            if x < 0.0 { return Err(CalcError::InvalidArgument("sqrt requires x >= 0".into())); }
            Ok(libm::sqrt(x))
        }
        ...
        BuiltinFn::Factorial => {
            ...
            if x < 0.0 || x != libm::floor(x) {
                return Err(CalcError::InvalidArgument("factorial requires a non-negative integer".into()));
            }
            let n = x as u64;
            if n > 20 { return Err(CalcError::Overflow); }
        ...
        BuiltinFn::NDiff => {
            ...
            if args.len() < 3 { return Err(CalcError::InvalidArgument("ndiff needs 3 args".into())); }
            let point = arg(args, 2, env)?;
            let h = 1e-7;
            let mut e = env.clone();
            let var = match &args[1] { Expr::Var(v) => v.clone(), _ => return Err(CalcError::InvalidArgument("ndiff: second arg must be a variable".into())) };
        ...
        BuiltinFn::NIntegrate => {
            if args.len() < 4 { return Err(CalcError::InvalidArgument("nintegrate needs 4 args".into())); }
            let var = match &args[1] { Expr::Var(v) => v.clone(), _ => return Err(CalcError::InvalidArgument("nintegrate: second arg must be variable".into())) };
        ...
        BuiltinFn::Sum => {
            ...
            if args.len() < 4 { return Err(CalcError::InvalidArgument("sum needs 4 args".into())); }
            let var = match &args[1] { Expr::Var(v) => v.clone(), _ => return Err(CalcError::InvalidArgument("sum: second arg must be variable".into())) };
        ...
        BuiltinFn::Product => {
            if args.len() < 4 { return Err(CalcError::InvalidArgument("product needs 4 args".into())); }
            let var = match &args[1] { Expr::Var(v) => v.clone(), _ => return Err(CalcError::InvalidArgument("product: second arg must be variable".into())) };
            let from = arg(args, 2, env)? as i64;
            let to   = arg(args, 3, env)? as i64;
            let mut acc = 1.0f64;
            let mut e = env.clone();
        ...
        BuiltinFn::Len => {
            match args.first() {
                ...
                _ => Err(CalcError::InvalidArgument("len expects a list or matrix".into())),
            }
        }
        BuiltinFn::Norm => {
            match args.first() {
                Some(Expr::List(items)) => {
                    let mut sum = 0.0f64;
                    for e in items { let v = eval_env(e, env)?; sum += v * v; }
                    Ok(libm::sqrt(sum))
                }
        ...
        BuiltinFn::If => {
            ...
            if cond != 0.0 { arg(args, 1, env) } else { arg(args, 2, env) }
        }
        BuiltinFn::Random => {
            let r = lcg_rand();
            if args.len() == 2 {
        ...
        BuiltinFn::Diff | BuiltinFn::Integrate | BuiltinFn::Taylor
        | BuiltinFn::Solve | BuiltinFn::Limit | BuiltinFn::Expand
        | BuiltinFn::Simplify => {
            Err(CalcError::InvalidArgument(format!("{} could not be evaluated numerically", f.name())))
        }
        BuiltinFn::Det | BuiltinFn::Tr | BuiltinFn::Transpose | BuiltinFn::Inv
        | BuiltinFn::Zeros | BuiltinFn::Ones | BuiltinFn::Eye
        | BuiltinFn::Dot | BuiltinFn::Cross | BuiltinFn::Range => {
            Err(CalcError::InvalidArgument(format!("{} requires matrix/list context", f.name())))
        }
    }
}
...
mod tests {
    ...
    fn user_fn_eval() {
        let mut env = Env::new();
        env.set_fn("square", crate::env::UserFn {
