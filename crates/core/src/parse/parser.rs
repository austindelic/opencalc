...
fn parse_def(src: &str) -> Result<Statement, CalcError> {
    ...
    let params: Vec<String> = {
        ...
        if ps.is_empty() {
            ...
            ps.split(',').map(|p| p.trim().to_string()).collect()
        }
    };
    let body_src = rest[rparen + 1..]
        .trim()
        .strip_prefix('=')
        ...
        loop {
            match self.peek() {
                Token::Plus => {
                    ...
                    lhs = flatten_add(lhs, rhs);
                }
                Token::Minus => {
                    self.advance();
                    let rhs = self.parse_term()?;
                    lhs = flatten_add(lhs, Expr::Neg(Box::new(rhs)));
                }
                _ => break,
            }
        }
        ...
        loop {
            match self.peek() {
                ...
                Token::Percent => {
                    self.advance();
                    let rhs = self.parse_unary()?;
                    lhs = Expr::Call(BuiltinFn::Mod, vec![lhs, rhs]);
                }
    ...
    fn parse_primary(&mut self) -> Result<Expr, CalcError> {
        match self.peek().clone() {
            Token::Number(n) => {
                ...
                if is_int {
                    Ok(Expr::Rat(Rational::from(n as i64)))
                } else {
                    Ok(Expr::Float(n))
                }
            }
            ...
            tok => Err(CalcError::ParseError(format!(
                "unexpected token: {:?}",
                tok
            ))),
        }
